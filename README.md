...
src/              The page. React 19, Tailwind v4, Motion.
  ...
  Console.jsx       The product demo (Triage, Patients, Analytics, Onboarding).
  charts.jsx        The demo's charts, drawn from theme tokens.
...
- **Type:** Mona Sans (headings), Geist (body), Geist Mono (numbers, times, chart figures). All are
  open-source (OFL-1.1) variable fonts from Fontsource, Latin subset, self-hosted.
- **Wordmark:** **RidgeVia** in Urbanist 800, pure black, and *Health* in Lexend 300, deep mint. Both
  are open-source stand-ins for SK Modernist and Nexa Pro, which are commercial. The flame is the
  mark, drawn in black. The wordmark lives in `Wordmark.jsx`.
- **Demo data:** everything in the Base Camp demo (messages, patients, charts) is synthetic and is
  labeled "Sample data" on the page. Do not present it as real results.
