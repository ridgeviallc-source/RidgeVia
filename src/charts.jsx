...
// The charts are drawn at the width they are given, in real pixels, so text stays the same size on a phone
// and a desktop instead of shrinking with a scaled drawing. Under COMPACT wide they switch to a phone layout:
// each label sits above its row and the plot takes the full width.
const MAX_W = 760
const COMPACT = 440
function useWidth() {
  ...
  return [ref, Math.min(width, MAX_W)]
}
...
export function MedTimeline({ rows }) {
  ...
  const left = compact ? 16 : 96
  const top = 22
  const rowH = compact ? 62 : 44
  ...
  return (
    <div ref={ref} className="w-full">
      {W > 0 && (
        <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Medication requests over twelve months, with one long gap in the metformin row" className="block">
          {months.map((m, i) => (
            <g key={m}>
          ...
          {rows.map((row, r) => {
            ...
            const cy = compact ? rowTop + 44 : rowTop + rowH / 2
            const segs = row.at.slice(1).map((m, i) => ({ a: row.at[i], b: m }))
            return (
              <g key={row.name}>
                <text x="0" y={compact ? rowTop + 14 : cy + 4} fontSize="13" fontWeight="600" style={{ fill: 'var(--foreground)' }}>
                  {row.name}
                </text>
                {segs.map(({ a, b }) => {
                  const gap = b - a > 1.5
                ...
                {row.at.map((m, i) => {
                  ...
                  return <motion.circle key={m} cx={x(m)} cy={cy} r="5.5" {...pop(0.15 + dot++ * 0.04)} style={{ fill: late ? 'var(--destructive)' : 'var(--health)', transformBox: 'fill-box', transformOrigin: 'center' }} />
                })}
              </g>
            )
          })}
...
export function BoxPlot({ groups }) {
  const [ref, W] = useWidth()
  const compact = W < COMPACT
  const left = compact ? 12 : 128
  const right = 22
  const top = compact ? 6 : 12
  const rowH = compact ? 78 : 60
  const H = top + groups.length * rowH + 32
  ...
  return (
    <div ref={ref} className="w-full">
      {W > 0 && (
        <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Box plot of days from inquiry to first visit, by referral source" className="block">
          {ticks.map((t) => (
            <g key={t}>
              <line x1={x(t)} x2={x(t)} y1={top} y2={H - 28} style={{ stroke: 'var(--foreground)' }} strokeOpacity="0.1" strokeDasharray="3 5" />
              <text x={x(t)} y={H - 9} textAnchor="middle" {...txt(11.5)}>
                {t}
              </text>
            </g>
          ))}
          {groups.map((g, i) => {
            ...
            const cy = compact ? rowTop + 50 : rowTop + rowH / 2
            const s = g.stats
            const d = i * 0.28
            return (
              <g key={g.name}>
                {compact ? (
                  <text x="0" y={rowTop + 14} fontSize="13" fontWeight="600" style={{ fill: 'var(--foreground)' }}>
                    {g.name}
                    <tspan dx="8" fontSize="11" fontWeight="400" style={{ fill: 'var(--muted-foreground)', ...DATA }}>
                      n = {s.n}
                    </tspan>
                  </text>
                ) : (
